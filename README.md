...
In this folder there will also be a file `summary_cellnumbers.txt` which summarizes the number of detected cells per sample in the RNA experiment, FB experiment and the intersect of both. If no FBs were present for that sample NAs are returned. The numbers are also presented as a barplot in `summary_cellnumbers.pdf`.
